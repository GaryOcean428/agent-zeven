...
var PerplexityAPI = /** @class */ (function (_super) {
    __extends(PerplexityAPI, _super);
    function PerplexityAPI() {
        var _this = _super.call(this) || this;
        if (!_this.config.apiKeys.perplexity) {
            thought_logger_1.thoughtLogger.log('warning', 'Perplexity API key not configured');
        }
        return _this;
    }
    PerplexityAPI.getInstance = function () {
        if (!PerplexityAPI.instance) {
            PerplexityAPI.instance = new PerplexityAPI();
        }
        return PerplexityAPI.instance;
    };
    PerplexityAPI.prototype.chat = function (messages, onProgress) {
                ...
                switch (_a.label) {
                    case 0:
                        if (!this.config.apiKeys.perplexity) {
                            throw new AppError_1.AppError('Perplexity API key not configured', 'API_ERROR');
                        }
                        _a.label = 1;
                    case 1:
                        _a.trys.push([1, 4, , 5]);
                        return [4 /*yield*/, fetch("".concat(this.config.services.perplexity.baseUrl, "/chat/completions"), {
                                method: 'POST',
                    ...
                    case 2:
                        response = _a.sent();
                        if (!response.ok) {
                            throw new AppError_1.AppError("Perplexity API error: ".concat(response.statusText), 'API_ERROR', { status: response.status });
                        }
                    ...
                    case 4:
                        error_1 = _a.sent();
                        thought_logger_1.thoughtLogger.log('error', 'Perplexity API request failed', { error: error_1 });
                        throw error_1 instanceof AppError_1.AppError ? error_1 : new AppError_1.AppError('Failed to communicate with Perplexity API', 'API_ERROR', { originalError: error_1 });
                    case 5: return [2 /*return*/];
                }
            });
        });
    };
    PerplexityAPI.prototype.handleStreamingResponse = function (body, onProgress) {
        return __awaiter(this, void 0, void 0, function () {
